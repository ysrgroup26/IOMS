...
<body>
    ...
    {{-- v2.12.0 (Product Finalization pass, Part 25 -- Document Audit):
         previously always used config('ioms.company') (hardcoded
         "YSR Systems") regardless of which tenant generated the
         document -- a real branding/tenant-identity leak in a
         multi-tenant document, inconsistent with the header just above
         (line 42) and with pdf/material-request.blade.php's own footer,
         which already gets this right. --}}
    <div class="footer-note">Generated by {{ $company->name ?? config('ioms.company') }} &middot; {{ now()->format('d M Y H:i') }}</div>
</body>
</html>
