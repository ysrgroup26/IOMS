...
export default function ControlledDocumentShow({ document: d, activities, canManage }) {
    ...
    return (
        <AuthenticatedLayout>
            ...
            <PageHeader title={<>{d.title}<StatusBadge value={d.status === 'effective' ? 'approved' : d.status === 'obsolete' ? 'secondary' : d.status} /></>} subtitle={<>{d.document_number} · v{d.version} {d.category && `· ${d.category}`} {d.department && `· ${d.department.name}`}</>}>
                {canManage && (
