...
export default function RfqShow({ rfq, activities, canManage }) {
    ...
    return (
        <AuthenticatedLayout>
            ...
            <PageHeader title={<>{rfq.rfq_number}<StatusBadge value={rfq.status} /></>} subtitle={<><Link href={route('purchase-requisitions.show', rfq.purchase_requisition.id)} className="hover:underline">{rfq.purchase_requisition.pr_number}</Link> {' · '}Deadline {new Date(rfq.quotation_deadline).toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' })} · Buyer {rfq.buyer?.name}</>}>
