    ...
    return (
        <PlatformLayout>
            ...
            <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h1 className="text-lg font-bold tracking-tight text-graphite-900">Plans / Editions</h1>
            ...
            <Card>
                ...
                <CardContent className="p-0">
                    {plans.length === 0 ? (
                        ...
                        <Table>
                            <TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Slug</TableHead><TableHead>Monthly</TableHead><TableHead>Yearly</TableHead><TableHead>Max Users</TableHead><TableHead>Max Companies</TableHead><TableHead>Status</TableHead><TableHead /></TableRow></TableHeader>
                            <TableBody>
                                {plans.map((p) => (
                                    <TableRow key={p.id}>
                                        <TableCell className="font-medium">{p.name}</TableCell>
                                        <TableCell><code className="text-xs">{p.slug}</code></TableCell>
                                        <TableCell>{p.price_monthly ? Number(p.price_monthly).toLocaleString() : '—'}</TableCell>
                                        <TableCell>{p.price_yearly ? Number(p.price_yearly).toLocaleString() : '—'}</TableCell>
        ...
        <Dialog open onOpenChange={(v) => !v && onClose()}>
            <DialogContent>
                ...
                <form onSubmit={submit} className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1.5"><Label>Price / Month</Label><Input type="number" min="0" step="0.01" value={data.price_monthly} onChange={(e) => setData('price_monthly', e.target.value)} /></div>
                        <div className="space-y-1.5"><Label>Price / Year</Label><Input type="number" min="0" step="0.01" value={data.price_yearly} onChange={(e) => setData('price_yearly', e.target.value)} /></div>
